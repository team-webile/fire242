...
class ManagerVotersExport implements FromCollection, WithHeadings, WithStyles
{
    protected $voters;
    protected $request;
    protected $columns;
    ...
    {  
        return $this->voters->map(function ($voter) {
            ...
            if (in_array('surveyed by', $this->columns)) {
                $row['Surveyed By'] = $voter->user ? $voter->user->name : '';
            }
            if (in_array('surveyed by email', $this->columns)) {
                $row['Surveyed By Email'] = $voter->user ? $voter->user->email : '';
            }
            ...
            if (in_array('email', $this->columns)) {
                $row['Email'] = ($voter->email) ? $voter->email : 'N/A';
            }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        foreach ($this->columns as $index => $column) {
            $letter = $this->getExcelColumnName($index + 1); // Excel is 1-based
            switch (strtolower($column)) {
                case 'id':
                case 'voter id':
    ...
    /**
     * Convert a column index to an Excel-style column letter (e.g. 1 = A, 27 = AA)
     */

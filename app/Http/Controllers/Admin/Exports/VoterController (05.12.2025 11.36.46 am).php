...
class VoterController extends Controller 
{
    ...
    return Excel::download(new VotersExport($voters, $request, $columns), 'Canvasser Voters In Survey.xlsx');  
   ...
   public function getVotersInSurvey(Request $request)
   {
    ...
    $query = Voter::with('user') 
    ->select('voters.*', 'constituencies.name as constituency_name', 'surveys.id as survey_id', 'surveys.created_at as survey_date','surveys.user_id','surveys.located','surveys.voting_decision','surveys.voting_for','surveys.is_died',
    'surveys.died_date','surveys.work_phone_code','surveys.work_phone','surveys.cell_phone_code','surveys.cell_phone','surveys.email', 'surveys.special_comments') 
    ->join('constituencies', 'voters.const', '=', 'constituencies.id')
    ->join(DB::raw("(
    ...
    return Excel::download(new SurveyVotersExport($voters, $request, $columns), 'Voters In Survey.xlsx');  
   ...
   public function getDiedVotersInSurvey(Request $request)
   {
      ...
      if(!empty($voting_for)){
            ...
            $query->where('surveys.voting_for', $party_name);
    }
    if($is_died !== null && $is_died !== ''){
        $query->where('surveys.is_died', $is_died);
    }
   ...
   public function getVotersNotInSurveyExport(Request $request)
   {
       ...
       if((!isset($constituencyId) || $constituencyId == '') && (!isset($constituencyName) || $constituencyName == '')){
        return response()->json([
            ...
            return Excel::download(new VotersExport($voters, $request, $columns), 'Voters Not In Survey.xlsx');  
   ...
   public function getVotersDiffAddress(Request $request)
   {
    ...
    $voters = $query->orderBy('id', 'desc')->get(); 
        ...
        return Excel::download(new VotersDiffAddressExport($voters, $request, $columns), 'Voters Diff Address.xlsx');  
   } 

...
class SurveyController extends Controller
{
    ...
    //     $cacheKey = "suggestions_{$type}_{$term}_" . implode('_', $constituencyIds);
    ...
    //         Cache::put($cacheKey, $suggestions, now()->addMinutes(60));
    //     }
...
// ... existing code ...
    ...
    //         try {
    //             // Validate unregistered voter data
    //             $unregisteredData = $request->validate([
    ...
    //             // Add user_id to unregistered data
    //             $unregisteredData['user_id'] = auth()->user()->id;

    //             // Create unregistered voter record
    //             $unregisteredVoter = UnregisteredVoter::create($unregisteredData);

             
    //             // $request->merge(['voter_id' => $unregisteredVoter->id]);
    //             return response()->json([
    //                 'success' => true,
    ...
    //             return response()->json([
    //                 'success' => false,
    //                 'message' => 'Validation failed for unregistered voter',
    //                 'errors' => $e->errors()
    //             ], 422);
    //         }
    //     }
    ...
    //             'work_phone' => 'nullable|string',
    //             'cell_phone' => 'nullable|string',
    //             'email' => 'nullable|email|unique:surveys,email',
    //             'special_comments' => 'nullable|string',
    //             'other_comments' => 'nullable|string',
    //             'voting_for' => 'required|string',
    //             'voted_in_2017' => 'required|boolean',
    //             'where_voted_in_2017' => 'nullable|string',
    //             'voted_in_house' => 'required|string',
    //             'voter_image' => 'nullable|image|mimes:jpeg,png,jpg|max:2048',
    //             'home_image' => 'nullable|image|mimes:jpeg,png,jpg|max:2048'
    //         ]);

    //         $validatedData['user_id'] = auth()->user()->id;
            
    //         // Handle voter image upload if provided
    //         if ($request->hasFile('voter_image')) {
    //             $voterImagePath = $request->file('voter_image')->store('surveys/voter_images', 'public');
           
    //             $validatedData['voter_image'] = $voterImagePath;
    //         }

    //         // Handle home image upload if provided  
    //         if ($request->hasFile('home_image')) {
    //             $homeImagePath = $request->file('home_image')->store('surveys/home_images', 'public');
    //             $validatedData['home_image'] = $homeImagePath;
    //         }
    ...
    public function show($id)
    {
        ...
        $survey = Survey::with('voter')->where('user_id', auth()->id())->where('voter_id',$id)->first();
